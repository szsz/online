...
TESTS=(
    ...
    "regression-writer-header-footer-remove|test-regression-writer-header-footer-remove.js|Regression: Writer header can be removed|Format → Page Style → Header tab → tick + Apply, reopen + untick + Apply, save, assert no <w:headerReference>. Pre-fix LOK modal DeleteHeaderDialog(null).run() returned RET_CANCEL, header survived.|shots-regression-writer-header-footer-remove"
    ...
    "regression-event-driven-docready|test-regression-event-driven-docready.js|Regression: Event-Driven Doc-Ready|kit emits docready: text frame; wasm-loader fireDocReady() routes through existing fan-out. Phase 1: polling parallel + [event-vs-poll] telemetry.|shots-regression-event-driven-docready"
    # Perf gate
