...
TESTS=(
    # Copy/paste cluster
    "cv-regression-mouse-select-copypaste|tests/content-viewer/test-cv-regression-mouse-select-copypaste.js|Regression: Mouse Select + Copy/Paste|Mouse click/double-click selection + copy/paste between 2 browsers|shots-regression-mouse-select-copypaste"
    "cv-regression-paste-coedit|tests/content-viewer/test-cv-regression-paste-coedit.js|Regression: Paste Co-Edit|Text and image paste in 2-browser co-edit|shots-regression-paste-coedit"
    "cv-regression-checkpoint-cursor-delete|tests/content-viewer/test-cv-regression-checkpoint-cursor-delete.js|Regression: Checkpoint Rotation + Cursor + Late-Join Delete|A inserts B selects A saves checkpoint rotates with cursor snapshot C joins B deletes all converge|shots-regression-checkpoint-cursor"
    "cv-regression-plaintext-paste|tests/content-viewer/test-cv-regression-plaintext-paste.js|Regression: Plaintext Paste|Plaintext-only paste path|shots-regression-plaintext-paste"
    "cv-latejoin-copypaste|tests/content-viewer/test-cv-latejoin-copypaste.js|Late Join + Copy/Paste|Late joiner receives all paste content (internal external text external image)|shots-latejoin-copypaste"
    "cv-e2e-copypaste|tests/content-viewer/test-cv-e2e-copypaste.js|E2E Copy/Paste|Real keyboard copy/paste between two browsers|shots-e2e-copypaste"
    ...
    "cv-regression-stylesview-overlap|tests/content-viewer/test-cv-regression-stylesview-overlap.js|Regression: Stylesview Layout|Notebookbar Styles entries occupy distinct grid cells (no overlap)|shots-regression-stylesview-overlap"
    "cv-regression-writer-navigator-flash|tests/content-viewer/test-cv-regression-writer-navigator-flash.js|Regression: Writer Navigator does not flash|Locks the Control.NavigatorPanel.ts idempotency guard — pre-fix the click listener was bound twice on cold open so a single click dispatched .uno:Navigator twice and the panel toggled open then closed.|shots-regression-writer-navigator-flash"
    "cv-regression-stylesview-preview|tests/content-viewer/test-cv-regression-stylesview-preview.js|Regression: Stylesview Visual Preview|Ribbon Styles entries visually preview their effect — Title bigger+bolder than Body Text; H1 > H2 > H3. Includes screenshots of ribbon + each entry + dropdown.|shots-regression-stylesview-preview"
