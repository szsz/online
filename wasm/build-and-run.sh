...
step_relay() {
    ...
    echo "    http://localhost:6931/browser/dist/cool.html?file_path=/test/example.odt"
    echo "    Then open console and run:"
    echo "      var s = document.createElement('script');"
    echo "      s.src = 'relay-host.js';"
    echo "      document.head.appendChild(s);"
    echo "      // wait a moment, then:"
    echo "      RelayHost.connect('ws://localhost:$RELAY_PORT/host?room=default');"
    echo ""
    echo "  Browser B (thin client):"
    echo "    http://localhost:6931/browser/dist/relay-client.html?relay=ws://localhost:$RELAY_PORT/client?room=default"
    echo ""
