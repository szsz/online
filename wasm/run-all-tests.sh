...
TESTS=(
    ...
    "regression-cool-html-substitution|test-regression-cool-html-substitution.js|Regression: cool.html template substitution in editor-static-server|editor-static-server.js must substitute %ACCESS_TOKEN%, %ACCESS_TOKEN_TTL%, %BRANDING_THEME% etc. at read time. Without it, the kit reads cool.html, finds literal %ACCESS_TOKEN%, appends it to /wasm/<id>?access_token=%ACCESS_TOKEN%&access_token_ttl=%ACCESS_TOKEN_TTL%, gets 404, exits at COOLWSD::run() entry — every kit-paint test (chart, caching, e2e-upload, singleuser, pptx-viewer, snapshot-milestones, ~25 more) hangs with shield-up. deploy-front-door.sh does the same substitution via sed at FD upload time.|none"
