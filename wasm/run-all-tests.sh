...
TESTS=(
    ...
    "regression-writer-header-footer-remove|test-regression-writer-header-footer-remove.js|Regression: Writer header can be removed via Page Style dialog|Format → Page Style → Header tab → Header on, Apply (adds header), reopen + untick Header on, Apply (removes header). Pre-fix: LO sw/source/uibase/wrtsh/wrtsh1.cxx ChangeHeaderOrFooter ran DeleteHeaderDialog(GetView().GetFrameWeld()).run() — in LOK pParent is null, the modal returns RET_CANCEL synchronously, bExecute flips to false, the SetFormatAttr(SwFormatHeader(false)) is skipped, the header survives. Fixed by LOK-gating bShowWarning = false at function entry. Test drives via real menu/toolbar/dialog clicks and asserts the saved docx has no <w:headerReference> and no word/header*.xml.|shots-regression-writer-header-footer-remove"
