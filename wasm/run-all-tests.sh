...
TESTS=(
    ...
    "regression-asset-map-shape|test-regression-asset-map-shape.js|Regression: __assetMap shape|cool.html __assetMap has all 10 expected keys, no extras, every value matches <base>.<8hex>.<ext> format — locks the cache-bust manifest contract|none"
    "regression-sw-heavy-patterns|test-regression-sw-heavy-patterns-cover-asset-map.js|Regression: SW HEAVY_PATTERNS coverage|every HEAVY __assetMap value is matched by a HEAVY_PATTERNS regex; every documented SMALL value is NOT — catches drift between cache-bust renames and SW caching list|none"
    ...
    "regression-cache-bust-idempotency|test-regression-cache-bust-idempotency.js|Regression: cache-bust inject idempotency|deployed cool.html has exactly 1 of each: COOL_CACHE_BUST_INJECT_BEGIN/END marker pair, IIFE that initializes window.__assetMap, wasm-loader script tag, relay-adapter script tag — pins the iter 26 fix that prevented the partial-update bug from freezing shim updates|none"
