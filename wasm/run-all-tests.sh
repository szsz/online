...
TESTS=(
    ...
    "cv-regression-print-button|tests/content-viewer/test-cv-regression-print-button.js|CV: Regression: Print button → blob URL|WASM-mode print short-circuits the dead /<urlPrefix>/<doc>/download/<id> HTTP route and reads print.pdf from Module.FS directly (CanvasTileLayer._onDownloadAsMsg). Asserts iframe.contentWindow.print() fires with a blob: URL whose body starts with %PDF-.|none"
    "cv-regression-console-noise-budget|tests/content-viewer/test-cv-regression-console-noise-budget.js|CV: Regression: Console-noise budget|Tripwire for the single-user cold-open console-log floor. Opens new.docx in ?singleuser, captures every console + pageerror, asserts total < 600 lines / 80 KB. Today ~1748 / 145 KB → test FAILS until Phases 1+2 of the cleanup (delete OverflowManager/OverflowGroup debug + gate JSDialog chatter behind ?debug=jsdialog) land. See ai/tasks/todo/console-log-cleanup.md.|none"
    "cv-regression-editing-session-spam|tests/content-viewer/test-cv-regression-editing-session-spam.js|CV: Regression: Editing-session console spam|Sibling of console-noise-budget but covers the LIVE-EDITING path the user actually triggers: open new.docx, click, type ~150 chars, scroll 10 ticks, quiesce 5s, count every console + pageerror. User-reported 2026-06-07 saw 500000+ lines of __emscripten_thread_mailbox_await stack-trace spam over a multi-hour session; cold-open test (1748→379 lines) totally missed it because per-layout-tick chatter (OverflowManager.onResize ~10/s) only stacks up during editing. Asserts total <650 lines / 90 KB.|none"
    "cv-regression-writer-navigator-flash|tests/content-viewer/test-cv-regression-writer-navigator-flash.js|CV: Regression: Writer Navigator does not flash|Clicks the floating Navigator icon; asserts the navigation-sidebar stays visible after a 2 s settle. Pre-fix createFloatingNavigatorBtn bound the click listener twice (initializeImpl runs twice on cold open), so one user click dispatched .uno:Navigator twice → kit toggled open then closed → panel flashed. Locks the Control.NavigatorPanel.ts idempotency guard.|none"
    "cv-regression-checkpoint-cursor-delete|tests/content-viewer/test-cv-regression-checkpoint-cursor-delete.js|CV: Regression: Checkpoint Rotation + Cursor + Late-Join Delete|A inserts, B selects, A saves → checkpoint rotates with cursor snapshot, C joins and sees B's selection, B deletes and all three converge|none"
    "cv-2browser|tests/content-viewer/test-cv-2browser.js|CV: 2-Browser Co-Edit|Two browsers co-editing with cursor sync|none"
    "cv-3browser|tests/content-viewer/test-cv-3browser.js|CV: 3-Browser Co-Edit|Three browsers simultaneous co-editing|none"
    "cv-formats|tests/content-viewer/test-cv-formats.js|CV: Format Tests|Opening multiple document formats|none"
    "cv-pptx|tests/content-viewer/test-cv-pptx.js|CV: PPTX / Impress|PowerPoint presentation opening in Impress|none"
    "cv-pptx-coedit|tests/content-viewer/test-cv-pptx-coedit.js|CV: PPTX Co-Editing|Two browsers co-editing a PowerPoint presentation|none"
    "cv-latejoin|tests/content-viewer/test-cv-latejoin.js|CV: Late Join|Late-joining user receives current document state|none"
    "cv-stress|tests/content-viewer/test-cv-stress.js|CV: Stress Test|Rapid concurrent edits under load|none"
    "cv-e2e-upload|tests/content-viewer/test-cv-e2e-upload.js|CV: E2E Upload & Co-Edit|Upload file, background preload, open, co-edit with 2nd browser|none"
    "cv-regression-sab-context|tests/content-viewer/test-cv-regression-sab-context.js|CV: Regression: SAB Browser Context|Same-context co-edit corrupts state; separate contexts converge|none"
    "cv-regression-checkpoint-timing|tests/content-viewer/test-cv-regression-checkpoint-timing.js|CV: Regression: Checkpoint Timing|Late joiners receive fresh checkpoint within 1.5s save budget|none"
    "cv-regression-stylesview-overlap|tests/content-viewer/test-cv-regression-stylesview-overlap.js|CV: Regression: Stylesview Layout|Notebookbar Styles entries occupy distinct grid cells (no overlap)|none"
    "cv-regression-stylesview-preview|tests/content-viewer/test-cv-regression-stylesview-preview.js|CV: Regression: Stylesview Visual Preview|Ribbon Styles entries visually preview their effect — Title bigger+bolder than Body Text; H1 > H2 > H3; Block Quotation italic. Custom styles fixture (test/data/custom-styles.docx) checked for kit enumeration; visual-preview for custom styles is EXPECTED FAIL until phase 2 (LO core manifest). Includes screenshots of ribbon + each entry + dropdown.|none"
    "cv-regression-dict-multi-lang|tests/content-viewer/test-cv-regression-dict-multi-lang.js|CV: Regression: dict-loader multi-language reactive load|exercises window.loadDictionary across 3 langs; asserts state.loaded grows AND second-call short-circuits to {cached:true} with no new fetch — locks the multi-language spellcheck substrate (task #196)|none"
    "cv-regression-dict-locale-resolve|tests/content-viewer/test-cv-regression-dict-locale-resolve.js|CV: Regression: dict-loader BCP 47 resolver|loadDictionaryForLocale maps runtime locale tags (en-US, fr-CA, de_AT, pt-BR…) to the manifest's actually-shipped lang via 4-tier match (exact → primary exact → primary-prefix → skipped); StatusBar's LanguageStatus handler depends on this for paragraph-language reactive dict loads|none"
    "cv-regression-mixed-lang-spellcheck|tests/content-viewer/test-cv-regression-mixed-lang-spellcheck.js|CV: Regression: mixed-language paragraph spellcheck plumbing|opens a docx with en-US/de-DE/fr-FR paragraphs; clicks into each in turn; asserts kit's .uno:LanguageStatus event flows through StatusBar → loadDictionaryForLocale → idempotent loadDictionary; verifies state.loaded grows on cross-paragraph cursor moves AND no new fetch on re-entry to a previously-loaded paragraph (locks iter 43 e2e)|none"
    "cv-regression-spellcheck-squiggle|tests/content-viewer/test-cv-regression-spellcheck-squiggle.js|CV: Regression: red spell-error squiggle paints on canvas|types three deliberate misspellings (Schmettrling fhsdkj qweryt) at top of an English paragraph; samples a 1200x140 band around the cursor row for red-channel pixels matching the squiggle color signature (r>150 g<90 b<90); asserts >=30 red pixels appear. Locks libreoffice-core-wasm PR #12 inftxt.cxx OnWin/LOK gate — pre-fix redPx==0, post-fix paints proper squiggles in tile rendering.|none"
    "cv-regression-spell-rightclick-suggest|tests/content-viewer/test-cv-regression-spell-rightclick-suggest.js|CV: Regression: right-click spelling suggestions (en/de/fr)|opens mixed-lang docx; for English (manuscrit) right-clicks the misspelled word and asserts a spelling context menu with hunspell suggestions appears, picks one, and confirms a re-right-click no longer flags the word; then primes the German + French paragraphs (cursor entry → lazy dict load), right-clicks a misspelled word in each (Woerter→Wörter, francais→français) and verifies suggestions + correction. Locks the LO LOK spell-menu emit + runtime dict registration/re-spell chain.|none"
    "cv-regression-language-picker-multilang|tests/content-viewer/test-cv-regression-language-picker-multilang.js|CV: Regression: language picker offers non-English languages|opens a docx, opens the status-bar language menu and asserts a non-English language (German/French/Italian/…) is offered — directly inline or via the More… dialog. Pre-fix getLanguages() returned only installed (English) dictionary locales so the user could never pick another language to trigger its lazy dict load; locks the LO getLanguages full-table emit + client manifest-narrowing.|none"
    "cv-regression-spell-language-switch|tests/content-viewer/test-cv-regression-spell-language-switch.js|CV: Regression: switching language re-spells with lazy dict (red squiggles appear)|types English-valid words, then switches the document to Spanish (inline favourite) and Hungarian (Set Language for Selection → More…), asserting red squiggle pixels appear on the canvas after each switch (the words are misspelled in both). Covers both menu paths + two languages. Pre-fix the lazily-loaded dict arrived after the spell pass and nothing re-checked the clean text; locks the LO re-spell-on-dict-install + SPELL_CORRECT_WORDS_AGAIN fix (LO 2026-06-27-110).|none"
    "cv-regression-iconview-rendercache-diag|tests/content-viewer/test-cv-regression-iconview-rendercache-diag.js|CV: Diag: Iconview rendersCache delivery|Captures Util.OnDemandRenderer cache-state distribution per controlId (gated by window.__l10nIconviewDebug from iter 10) — data for the ribbon-style preview fix (#195/iter 6 deferred)|none"
    "cv-regression-first-client-overwrite|tests/content-viewer/test-cv-regression-first-client-overwrite.js|CV: Regression: First Client Overwrite|First client's activation checkpoint must not save stale/blank content over a newer relay state|none"
    "cv-regression-fontsize-dropdown|tests/content-viewer/test-cv-regression-fontsize-dropdown.js|CV: Regression: Font-Size Dropdown|Bug 1 single-tab — font-size dropdown shows the full size list (not just one option)|none"
    "cv-regression-latejoin-offline-unsaved|tests/content-viewer/test-cv-regression-latejoin-offline-unsaved.js|CV: Regression: Late Join — A Offline + Unsaved|A types unsaved, A goes offline, B joins; B sees A's edits via relay history|none"
    "cv-regression-latejoin-overwrite|tests/content-viewer/test-cv-regression-latejoin-overwrite.js|CV: Regression: Late Join Overwrite|Late joiner must NOT overwrite document with blank/stale content during checkpoint|none"
    "cv-regression-latejoin-unsaved|tests/content-viewer/test-cv-regression-latejoin-unsaved.js|CV: Regression: Late Join Unsaved|Late joiner with UNSAVED edits (no Ctrl+S from first browser) must converge|none"
    "cv-regression-real-copypaste|tests/content-viewer/test-cv-regression-real-copypaste.js|CV: Regression: Real Copy/Paste|REAL Ctrl+C/V flow (no JS-injected paste); validates clipboard wiring end-to-end|none"
    "cv-regression-calc-impress-edits|tests/content-viewer/test-cv-regression-calc-impress-edits.js|CV: Regression: Calc/Impress Edit Propagation|Calc cell edits and Impress text edits fire invalidatetiles AND propagate to remote peers|none"
    "cv-regression-select-delete-coedit|tests/content-viewer/test-cv-regression-select-delete-coedit.js|CV: Regression: Select+Delete Co-Edit|A double-clicks a word + presses Delete; B must converge to the same shorter doc (deletion must propagate)|none"
    "cv-regression-delete-key-coedit|tests/content-viewer/test-cv-regression-delete-key-coedit.js|CV: Regression: Delete Key Co-Edit|Delete key generates removetextcontext (not key); the relay-adapter must recognize it as user-input and forward to peers|none"
    "cv-regression-docname-switch|tests/content-viewer/test-cv-regression-docname-switch.js|CV: Regression: Doc Name Updates on Switch|Hot-switch to a different doc must update the title bar from the old name to the new one|none"
    "cv-regression-paste-coedit|tests/content-viewer/test-cv-regression-paste-coedit.js|CV: Regression: Paste Co-Edit|Text and image paste in 2-browser co-edit (docx); text in both browsers, image embedded in saved file|none"
    "cv-coedit-convergence-churn|tests/content-viewer/test-cv-coedit-convergence-churn.js|CV: Co-edit: convergence + churn|Multi-browser co-edit: A/B/C/D join, type, paste, leave, late-join; every live browser converges on char count at each step, no checkpoint mismatch/abort|none"
    "cv-coedit-concurrent|tests/content-viewer/test-cv-coedit-concurrent.js|CV: Co-edit: concurrent edits|Two/three browsers type SIMULTANEOUSLY (no wait) with C joining + B leaving mid-stream; summed char count must converge everywhere|none"
    "cv-coedit-feature-shape|tests/content-viewer/test-cv-coedit-feature-shape.js|CV: Co-edit: insert-shape propagation|A inserts a rectangle → B's canvas changes; B inserts → A's changes; C late-joins and renders both shapes|none"
    "cv-coedit-feature-table|tests/content-viewer/test-cv-coedit-feature-table.js|CV: Co-edit: insert-table propagation|A inserts a 3x3 table → B renders it; B inserts 2x2 → A renders it; C late-joins and renders both tables|none"
    "cv-coedit-formatting|tests/content-viewer/test-cv-coedit-formatting.js|CV: Co-edit: formatting propagation|Bold/italic/font-size applied in one browser render in the peer; late-joiner inherits formatting|none"
    "cv-coedit-rejoin-storm|tests/content-viewer/test-cv-coedit-rejoin-storm.js|CV: Co-edit: rejoin storm|A edits continuously while a participant repeatedly leaves + rejoins across checkpoint rotations; every rejoin converges|none"
    "cv-coedit-spell-correct|tests/content-viewer/test-cv-coedit-spell-correct.js|CV: Co-edit: language + spellcheck + spell-correct|German paragraph spell-checks in German; both browsers show the spelling menu; A picks a suggestion and the correction converges to B|none"
    "cv-coedit-latejoin-checkpoint-retry|tests/content-viewer/test-cv-coedit-latejoin-checkpoint-retry.js|CV: Co-edit: late-join w/ unsaved edits|A makes a few small unsaved edits; B late-joins and must activate without a runaway checkpoint-download wait, converge to A, and edit back — guards the fetchCheckpointWithRetry timeout/retry fix|none"
    "cv-coedit-spell-correct-latejoin|tests/content-viewer/test-cv-coedit-spell-correct-latejoin.js|CV: Co-edit: spell-correct then late-join|A right-clicks a misspelled word, picks a suggestion, deselects + saves; a late-joiner C must converge to A's corrected char count (guards the checkpoint-rotation invariant that keeps unpersisted edits replayable)|none"
    "cv-coedit-convergence-conflict|tests/content-viewer/test-cv-coedit-convergence-conflict.js|CV: Co-edit: convergence under conflict|Adversarial convergence: two users type at the SAME cursor position simultaneously; one select-all-deletes while another types (+ late-join); three users type at once. Every live participant + late-joiner converges to one identical char count, no checkpoint mismatch|none"
    "cv-coedit-churn-load-budget|tests/content-viewer/test-cv-coedit-churn-load-budget.js|CV: Co-edit: churn storm + load budget|Host drives continuous edits while up to 7 users join (join-storms), edit, and leave across rounds + a final full-replay joiner. Every join's cold load->ready time is asserted under a 60s hard budget (catches the checkpoint-download hang class + load regressions); each round all live users converge|none"
    "cv-coedit-latejoin-manymsg|tests/content-viewer/test-cv-coedit-latejoin-manymsg.js|CV: Co-edit: late-join after many unsaved msgs|Host accumulates ~90 unsaved messages (typing + paragraph split + concurrent peer edit); a fresh late-joiner must reconstruct the full state and not lose edits to the switchdocument reload discarding replay applied to the prewarm blank|none"
    "cv-e2e-copypaste|tests/content-viewer/test-cv-e2e-copypaste.js|CV: E2E Copy/Paste|Real keyboard Ctrl+C/V from viewer: type, internal copy/paste, external text paste, external image paste, internal after external. Auto-generates detailed HTML report with screenshots + clipboard state at every step|none"
    "cv-latejoin-copypaste|tests/content-viewer/test-cv-latejoin-copypaste.js|CV: Late Join + Copy/Paste|Late joiner receives all paste content (internal, external text, external image) from first browser|none"
    "cv-regression-plaintext-paste|tests/content-viewer/test-cv-regression-plaintext-paste.js|CV: Regression: Plain Text Paste|Pasting unformatted text/plain (no HTML) from terminal or Notepad must work via all paths: blob, string, and paste event|none"
    "cv-regression-mouse-select-copypaste|tests/content-viewer/test-cv-regression-mouse-select-copypaste.js|CV: Regression: Mouse Select + Copy/Paste|Mouse click/double-click selection + copy/paste between 2 browsers|none"
    "cv-regression-rightclick-copypaste|tests/content-viewer/test-cv-regression-rightclick-copypaste.js|CV: Regression: Right-click Copy + Paste|First E2E coverage of the right-click context menu's Copy path. Real puppeteer right-mouse-click on selected canvas text, real-click on the Copy menu item, then Ctrl+V to confirm the kit-side clipboard chain works. Documents the smoking gun where navigator.clipboard.readText() returns empty after right-click Copy (external clipboard write silently fails due to unhandled GET /cool/clipboard in wasm-loader.js)|none"
    "cv-regression-xlsx-sheet-tabs-rename|tests/content-viewer/test-cv-regression-xlsx-sheet-tabs-rename.js|CV: Regression: Calc Sheet TABS visibility + rename|Each sheet must have a visible, clickable tab in the bottom strip. Real-click on tab #2 activates that sheet. Double-click on tab #1 opens the rename input modal. Right-click on tab #1 → 'Rename Sheet…' opens the same modal. Catches the bug where SheetsBar (Control.SheetsBar.js:95 parentContainer.replaceChildren()) wipes the spreadsheet-tabs-container that Control.Tabs._initialize() just appended to #spreadsheet-toolbar — SheetsBar is constructed twice via initializeSpecializedUI('spreadsheet'), the second construction wipes the tab DOM, leaving only the nav arrows (|none"
    "cv-regression-pptx-transitions-iconview|tests/content-viewer/test-cv-regression-pptx-transitions-iconview.js|CV: Regression: PPTX Transitions iconview|Each tile in the Impress Transition tab's iconview must have a real (>=30 px) height. Catches the Notebookbar.ImpressTransitionTab.ts regression where ondemand entries lacked width/height and collapsed to 2-pixel flat strips, leaving the entire transitions ribbon visually empty|none"
    "cv-regression-impress-transition-click|tests/content-viewer/test-cv-regression-impress-transition-click.js|CV: Regression: PPTX Transition click applies|Clicking a transition tile in the Impress Transitions tab must apply the transition (not just select the tile). Catches the Notebookbar.ImpressTransitionTab.ts regression where the iconview lacked singleclickactivate, so the click handler in Widget.IconView.ts only fired the 'select' builderCallback and never the 'activate' that propagates to the kit|none"
    "cv-regression-pptx-save-no-abort|tests/content-viewer/test-cv-regression-pptx-save-no-abort.js|CV: Regression: PPTX Save must not abort kit|.uno:Save through relay-adapter must trigger saveAndUploadCheckpoint() WITHOUT also forwarding to the kit. The kit's wsd/DocumentBroker.cpp:5284 has an assertion that aborts on uno .uno:Save; the user-visible symptom was 'save fails, only Discard available' after any edit + Ctrl+S|none"
    "cv-regression-impress-area-dialog|tests/content-viewer/test-cv-regression-impress-area-dialog.js|CV: Regression: Impress Area dialog must not crash kit|Right-click → Area → pick colour → OK in Impress must not trigger 'memory access out of bounds' from Browser_mainLoop. Hypothesis: sd/source/ui/func/fuarea.cxx:62 StartExecuteAsync callback captures pView+pViewShell by raw pointer and runs after FuArea has unwound. Three deterministic cycles to attest reproduction rate.|none"
    "cv-regression-writer-insert-shape-area|tests/content-viewer/test-cv-regression-writer-insert-shape-area.js|CV: Regression: Writer insert-shape + Area dialog|Two bugs: (A) Insert→Shapes popup must contain Rounded Rectangle + Rounded Square entries; (B) right-click Area → colour → OK on an inserted shape must not crash kit with memory-OOB. Bug B same LO trap signature as impress-shape-area-cant-be-saved; Writer dispatch site is sw/source/uibase/shells/drawdlg.cxx:121 (SwDrawShell::ExecDrawDlg SID_ATTRIBUTES_AREA, StartExecuteAsync captures pSh+pView by raw pointer). Three deterministic cycles.|none"
    "cv-regression-writer-shape-area-oom|tests/content-viewer/test-cv-regression-writer-shape-area-oom.js|CV: Regression: Writer shape Area dialog must not OOM kit|Right-click → Area... on an inserted shape in Writer must not trigger Aborted(Cannot enlarge memory arrays …) → Uncaught RuntimeError: unreachable. WASM is pinned at TOTAL_MEMORY=1GB (solenv/gbuild/platform/EMSCRIPTEN_INTEL_GCC.mk:18) with no ALLOW_MEMORY_GROWTH; the SvxAreaTabDialog ctor allocates past the 1 GB cap. User-reported 2026-06-02 on internal editor build 2026-06-02-174734. Tripwire test — expected FAIL until the OOM is fixed (raise TOTAL_MEMORY, enable ALLOW_MEMORY_GROWTH, or fix the LO codepath).|none"
    "cv-regression-area-palette|tests/content-viewer/test-cv-regression-area-palette.js|CV: Regression: Area dialog Colors tab shows real palettes|After the PaletteManager OOB fix the Area dialog opens but the colour grid is blank and the Palette dropdown lists only the 3 dynamic palettes (Custom/Theme/Document). Root cause: static/CustomTarget_emscripten_fs_image.mk packaged colorpage.ui but not share/palette/*.soc, so PaletteManager loaded zero built-in palettes (LO PR #40 adds them to the emscripten preload list). Inserts a rectangle, right-click → Area → reads the rendered palette <select> options and asserts >3 options incl. a built-in (standard/libreoffice/html/material/tonal), then selects one + clicks a swatch and asserts the New colour changes. Tripwire — FAILS pre-fix.|none"
    "cv-regression-copy-paste-suite|tests/content-viewer/test-cv-regression-copy-paste-suite.js|CV: Regression: Copy/paste suite (phase 1)|Unified copy/paste regression suite consolidating the 9 previously-separate clipboard tests. Phase 1 (2026-06-01): 1 ported use-case (Ctrl+A→C→End→V baseline) + 13 TODO placeholders enumerating the remaining migration work. Each subsequent phase ports another use-case from its source test file; eventually replaces test-{regression-real-copypaste, regression-mouse-select-copypaste, regression-plaintext-paste, regression-paste-coedit, regression-rightclick-copypaste, regression-paste-table, singleuser-copy-paste, e2e-copypaste, late-join-copypaste}. Per-use-case pass/fail in the report.|none"
    "cv-regression-font-change-ui|tests/content-viewer/test-cv-regression-font-change-ui.js|CV: Regression: Font Change via UI|Single-user, double-click word + change font via notebookbar dropdown — UNO command dispatched and font value updates|none"
    "cv-regression-writer-header-footer-remove|tests/content-viewer/test-cv-regression-writer-header-footer-remove.js|CV: Regression: Writer header/footer remove via Page Style|Smoke round-trip: Format → Page Style → Header tab → tick + Apply → Ctrl+S → unzip saved docx, assert <w:headerReference> + word/header*.xml present. Re-open dialog, untick + Apply → Ctrl+S → assert both removed. Pre-LO-PR-#30, ChangeHeaderOrFooter ran DeleteHeaderDialog(null).run() in LOK mode (GetFrameWeld() returns null), got RET_CANCEL → bExecute=false → SetFormatAttr that flips header off was skipped → header survived. Drives entirely through real puppeteer mouse + keyboard (no sendUnoCommand / no app.dispatcher.dispatch / no page.evaluate(()=>el.click())).|none"
    "cv-regression-double-click-word-copypaste|tests/content-viewer/test-cv-regression-double-click-word-copypaste.js|CV: Regression: double-click word → copy → paste|Smoke port of singleuser-copy-paste case 3 into the regression suite. Type 'hello world', double-click a word, Ctrl+C → Ctrl+End → Ctrl+V; assert StateWordCount grew (some text pasted). Isolates the kit-side word-select + clipboard chain from the multi-case singleuser run where prior cases pre-populate the clipboard.|none"
    "cv-regression-external-image-paste|tests/content-viewer/test-cv-regression-external-image-paste.js|CV: Regression: external image paste (PNG) routes to local Kit|Smoke port of singleuser-copy-paste case 7 into the regression suite. Single-user mode, real puppeteer Ctrl+V on canvas with a 1x1 PNG on the OS clipboard. Asserts the relay-adapter dispatched insertfile to local Kit (NOT through WS) and Kit's handleMessage saw insertfile graphic. Tripwire for the iter9 bug where insertfile was incorrectly relayed in single-user mode.|none"
    "cv-regression-mouse-drag-copypaste|tests/content-viewer/test-cv-regression-mouse-drag-copypaste.js|CV: Regression: mouse-drag select → copy → paste|Smoke port of singleuser-copy-paste case 4 into the regression suite. Type 'drag-target', Ctrl+Home, mouse-drag horizontally across the text, Ctrl+C → Ctrl+End → Ctrl+V; assert StateWordCount grew (some text pasted). Isolates the kit-side drag-select + clipboard chain. Known coord-pitfall: drag-coords must be computed BEFORE any Ctrl+End scroll, hence drag at top.|none"
    "cv-regression-ctrl-x-cut-restore|tests/content-viewer/test-cv-regression-ctrl-x-cut-restore.js|CV: Regression: Ctrl+X cut → Ctrl+V restores (TRIPWIRE)|Tripwire for the LO-side kit .uno:Cut bug (proposal ai/proposals/promoted/ctrl-x-isolated-single-user-clipboard.md). Type 'cut-test-abc', Ctrl+Home + Shift+ArrowRight ×3, Ctrl+X, Ctrl+V; assert doc restored to before-cut count. EXPECTED to FAIL until LO fix lands: in the multi-case singleuser run this passes accidentally (prior case left OS clipboard valid); ISOLATED, the kit's .uno:Cut doesn't write the cut bytes to the OS clipboard, so Ctrl+V has nothing to restore.|none"
    "cv-regression-bulk-open-ignored|tests/content-viewer/test-cv-regression-bulk-open-ignored.js|CV: Regression: Bulk Open (test/samples/ignored)|Discovers every file in test/samples/ignored/, opens them sequentially in a single Chromium, types 'hello world' at the top via real keyboard, verifies via canvas pixel-diff + StateWordCount delta. Produces an HTML report with per-file open times + before/after screenshots at /tmp/static-deploy/public/reports/regression-bulk-open-ignored.html. Corpus is gitignored — local-only test; exits 0 regardless of per-file verify result so the run never blocks the suite.|none"
)
