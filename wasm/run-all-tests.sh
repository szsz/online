...
TESTS=(
    ...
    "coedit-latejoin-checkpoint-retry|tests/coedit/test-coedit-latejoin-checkpoint-retry.js|Co-edit: late-join w/ unsaved edits|A makes a few small unsaved edits; B late-joins and must activate without a runaway checkpoint-download wait, converge to A, and edit back — guards the fetchCheckpointWithRetry timeout/retry fix|none"
    "coedit-spell-correct-latejoin|tests/coedit/test-coedit-spell-correct-latejoin.js|Co-edit: spell-correct then late-join|A right-clicks a misspelled word, picks a suggestion, deselects + saves; a late-joiner C must converge to A's corrected char count (guards the checkpoint-rotation invariant that keeps unpersisted edits replayable)|none"
    "coedit-convergence-conflict|tests/coedit/test-coedit-convergence-conflict.js|Co-edit: convergence under conflict|Adversarial convergence: two users type at the SAME cursor position simultaneously; one select-all-deletes while another types (+ late-join); three users type at once. Every live participant + late-joiner converges to one identical char count, no checkpoint mismatch|none"
    "coedit-churn-load-budget|tests/coedit/test-coedit-churn-load-budget.js|Co-edit: churn storm + load budget|Host drives continuous edits while up to 7 users join (join-storms), edit, and leave across rounds + a final full-replay joiner. Every join's cold load->ready time is asserted under a 60s hard budget (catches the checkpoint-download hang class + load regressions); each round all live users converge|none"
    "coedit-latejoin-manymsg|tests/coedit/test-coedit-latejoin-manymsg.js|Co-edit: late-join after many unsaved msgs|Host accumulates ~90 unsaved messages (typing + paragraph split + concurrent peer edit); a fresh late-joiner must reconstruct the full state and not lose edits to the switchdocument reload discarding replay applied to the prewarm blank|none"
