...
TESTS=(
    ...
    "regression-wasm-endpoint-viewer|test-regression-wasm-endpoint-viewer.js|Regression: /wasm/<name> hosted by viewer|POST + GET FILE_STORAGE_URL/wasm/<name> works and GET carries CORP:cross-origin so the editor iframe (COEP:require-corp) can load it — locks the FD-migration move of the dynamic upload endpoint from editor to viewer|none"
