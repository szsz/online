...
TESTS=(
    ...
    "regression-spell-language-switch|tests/regression/test-regression-spell-language-switch.js|Regression: switching language re-spells with lazy dict (red squiggles appear)|types English-valid words, switches the document to Spanish via the status-bar menu, and asserts red squiggle pixels appear on the canvas (the words are misspelled in Spanish). Pre-fix the lazily-loaded dict arrived after the spell pass and nothing re-checked the clean text; locks the LO re-spell-on-dict-install + SPELL_CORRECT_WORDS_AGAIN fix (LO 2026-06-27-110).|shots-regression-spell-language-switch"
    ...
    "regression-docswitch-dialogs|tests/regression/test-regression-docswitch-dialogs.js|Regression: 2nd doc in same tab breaks notebookbar/dialogs (TRIPWIRE)|User-reported 2026-06-19 (incognito, not cache): the first doc opened in a tab works (Insert ribbon, Symbol dialog, shape Area palette all open), but after opening a SECOND doc in the SAME tab (doc-switch via #file= hashchange) the notebookbar tab switching breaks — clicking Insert no longer activates the Insert ribbon, so Shapes can't be reached, no shape can be inserted, and right-click → Area palette never appears (cursor also buggy). Opens DOC A (Insert→rectangle→right-click→Area→palette, all PASS as a sanity gate), switches to DOC B in the same tab, repeats — DOC B asserts FAIL today, PASS once the doc-switch notebookbar regression is fixed. Pure visible-UI input. Expected FAIL until fixed.|shots-regression-docswitch-dialogs"
